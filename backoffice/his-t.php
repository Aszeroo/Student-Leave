...
<form method="POST" action="../report.php" target="_blank" style="display:inline;">
    <input type="hidden" name="education_id" value="<?= htmlspecialchars($education_id) ?>">
    <input type="hidden" name="sub_major_fullname" value="<?= htmlspecialchars($sub_major_fullname) ?>">
    <input type="hidden" name="classroom_id" value="<?= htmlspecialchars($classroom_id) ?>">
    <button type="submit" class="sdt-btn-print">
...
<div id="studentList">
<table class="his-stu-table"  id="sdt-table">
  <thead>
    <tr>
      <th>ลำดับ</th>
      <th>รหัสนักศึกษา</th>
...
<tbody id="sdt-tbody">
    ...
    if (!empty($students)) {
        ...
        foreach ($students as $std) {
            $stmt2 = $pdo->prepare("SELECT SUM(leave_period) AS total_leave FROM leave_requests WHERE student_id = ? AND status = 'approved'");
    ...
    ?>
            <tr>
                ...
                <td><?= htmlentities($total_leave) ?></td>
                <td><?= htmlentities($total_leave * 8) ?></td>
                <td>
                    <a href="his-detail.php?id=<?= urlencode($teacher['teacher_id']) ?>&student_id=<?= urlencode($std['student_id']) ?>">
